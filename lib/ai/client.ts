...
// in practice, since plain-text JSON occasionally got cut off mid-string and failed to parse
// (see the `triage.ai_call_failed` audit entries saying "Unterminated string in JSON"). Forcing
// the tool call avoids that failure mode entirely.
//
...
export async function callClaudeForJson<T>(prompt: string, schema: ZodType<T>): Promise<T> {
  ...
  try {
    const response = await getClient().messages.create({
      ...
      // each with several fields of reasoning, so this budget just makes sure we never cut a
  ...
  // This happens most often when a field name (e.g. "shortlisted") echoes the shape shown to
  // it elsewhere in the prompt. If the input is a single string field that itself parses as valid JSON,
