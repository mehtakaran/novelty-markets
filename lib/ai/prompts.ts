...
1. It's big enough that a broad audience would care — not a small local/niche story.
...
3. It's not vague or open-ended. "There might be a big announcement soon" — skip it, too
   vague. "Will the product ship by its stated Q3 deadline" — that's fine, it's checkable.
...
Multiple raw items (news / social / trending-market) may describe the same underlying event —
if so, produce at most one shortlisted candidate for it, and prefer the item that gives you the
clearest resolution date/criteria as the sourceItemId to cite.
...
it — for example an ongoing news story that could look distasteful as a betting market,
something that could embarrass the business, or a real legal/reputational risk that isn't
...
framing → higher probability, toss-up or speculative framing → closer to 0.5, long-shot
framing → lower probability. Say clearly in your reasoning that this is a soft guess based on
coverage, not a real market probability, and that there's no reference market behind it.`;
}
